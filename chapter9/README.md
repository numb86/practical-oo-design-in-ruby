...
ダックタイプに対するテストの題材として、第5章の`Preparer`ダックタイプを扱う。  
その実装は[5_1_2.rb](../chapter5/5_1_2.rb)。

`Preparer`ロールの担うオブジェクトは、`Mechanic`、`Driver`、`TripCoordinator`の3つ。  
その全てに対して、`Preparer`ロールとして正しく振る舞うかどうかのテストを書く。  
具体的には、`Preparer`ロールのパブリックインターフェースである`prepare_trip`メソッドを持つかどうかをテストする。  
そのテストは、一度だけ書かれ、全ての担い手がそれを共有するようにすべき。  
そのためにモジュールを用いる。以下がその例。
...
モジュールとして定義することで、テストを一度だけ書けば、ロールを担う全てのオブジェクトで再利用することが出来る。

ロールの担い手だけでなく、それを利用する側のテストも書くべき。`5_1_2.rb`の例だと、`Trip`が`prepare_tripe`を送信しているかのテスト。  
そのようなテストは、モックを使うことで実装できる。

ロールに対する共有可能なテストを書くことで、ロールを担っているテストダブルにもテストを書けるようになる。
例えば、`9.2`の`DiameterDouble`。  
テストダブルにもテストを書くことで、インターフェースの変更によってテストダブルが壊れてしまった際に正しくそれを検知できる。

## 9.6 継承されたコードをテストする

継承されたコードの題材として、第6章の`Bicycle`を扱う。  
実装は[6_5_2.rb](../chapter6/6_5_2.rb)。

`7.2`に書いたように、サブクラスは、スーパークラスのインターフェースを全て満たしている必要がある。  
そのため、階層構造に属する全てのオブジェクトに対して、そのインターフェースを満たしているかのテストを書く。  
これは、インターフェースのテストをモジュールとして書き、それぞれのクラスのテストでインクルードすれば実現できる。  
このことには、インターフェースを文書化する効果もある。

`Bicycle`はサブクラスに対して`default_tire_size`を持つことを求めているので、それについてもテストする。

スーパークラスとサブクラスがそれぞれ持つ固有の振る舞いに対しても、テストを書く。  
スーパークラスはインスタンスを作ることを想定していないので、テストを書きづらい場合がある。そのようなときは、テストのためだけに使うサブクラスを、スタブとして作ればよい。
...
テストがなければ、アプリケーションに対する理解も変更も困難になってしまう。  
適切に設計され、かつテストも書かれているアプリケーションは、理解も拡張も楽しいものとなる。
